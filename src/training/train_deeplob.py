...
def run_epoch(model, loader, optimizer, criterion, train=True, scheduler=None, should_stop=None):
...
def main():
    ...
    # CSV logging setup
    csv_path = Path(f"metrics_deeplob.csv")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        train_loss, train_f1, train_cm, train_metrics = run_epoch(
            model, train_loader, optimizer, criterion, train=True, scheduler=scheduler, should_stop=should_stop
        )
        val_loss, val_f1, val_cm, val_metrics = run_epoch(
        ...
        csv_row = {"epoch": epoch, "train_loss": train_loss, "train_macro_f1": train_f1, 
                   "val_loss": val_loss, "val_macro_f1": val_f1}
        csv_row.update({f"val_{k}": v for k, v in val_metrics.items()})
